...
> I used Flexbox for different divs and IDs so that the content of the website adjusts seamlessly when opened on a phone, laptop, or computer. For devices with phone aspect ratio, instead of a navigation bar a hamburger button will appear. Additionally, I implemented media queries, which can be found in the `mediaqueries.css` file, to handle this responsiveness. Media queries allow for the application of specific styles based on the device's screen width, ensuring a consistent and user-friendly experience across various platforms.
